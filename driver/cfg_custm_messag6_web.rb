...
begin 
  ...
  while(row <= rows)
    ...
    # value is a collection of all inputs to be written for a given row
   ...
   #Is there a popup expected? 
    ...
    y = 0
    type[0..9].each do|item|
      ws.Range("#{col}#{row}")['Value'] = g.checkbox(item)if y < 8
      ws.Range("#{col}#{row}")['Value'] = item.value if y > 7 && value[7] == 'set'
      col = col.next
      y += 1
      puts "enter to continue"; gets # pause each loop for debug
      p y # for debug
    end
