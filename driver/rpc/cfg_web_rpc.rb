...
begin 
  ...
  while(row <= rows)
    ...
    web_mode = ((g.websrvr.value).to_s.insert(0,'"').insert(9,'"'))
    puts "Web Server = #{web_mode}"
    ws.Range("bc#{row}")['Value'] = web_mode
    
    web_mode = (web_mode.delete'"').to_s
    puts"Web-server mode = #{web_mode}"
    
    if(web_mode == "00000001")
      ws.Range("bd#{row}")['Value'] = g.httpport.value
    elsif(web_mode == "00000002")
